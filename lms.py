...
# Adding book information to database
    def adddata(self):
        a=self.aid.get()
        b=self.aname.get()
        c=self.aauthor.get()
...
def Database():
    ...
    if cursor.fetchone() is None:
        cursor.execute("INSERT INTO `login` (username, password) VALUES('ash', 'root')")
        conn.commit()
